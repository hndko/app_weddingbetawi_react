...
export const OpeningCover: React.FC<{ onOpen: () => void }> = ({ onOpen }) => {
  ...
  return (
    <motion.div
      className="absolute inset-0 z-50 flex flex-col items-center justify-between text-center px-6 overflow-y-auto overflow-x-hidden no-scrollbar pt-10 pb-8 bg-gradient-to-b from-[#2A080C] via-[#4A0E13] to-[#1F0508] text-[#FAF9F5]"
    ...
    >
      ...
      >
        ...
        >
          <MahkotaSiger size={110} color="#D4AF37" accentColor="#7B1122" />
        </motion.div>
      ...
      >
        <div className="bg-[#1F0508]/85 backdrop-blur-md px-5 py-5 rounded-2xl border border-[#D4AF37]/40 shadow-xl relative overflow-hidden flex flex-col items-center">
          ...
          <button
            type="button"
            onClick={onOpen}
            className="w-full bg-gradient-to-r from-[#B38B22] via-[#D4AF37] to-[#B38B22] text-[#1F0508] font-semibold py-3 px-6 rounded-full flex items-center justify-center gap-2 hover:brightness-110 transition-all duration-300 shadow-md hover:shadow-lg cursor-pointer text-xs tracking-wide uppercase"
          >
            <MailOpen size={16} />
